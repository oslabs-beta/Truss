...
exports.renderGraphAsDot = renderGraphAsDot;
const LAYER_COLORS = [
    "#dbeafe", // blue
    "#fef3c7", // amber
    "#dcfce7", // green
...
function renderGraphAsDot(graph, layers, violations) {
    const lines = [];
    lines.push("digraph G {");
    lines.push('  rankdir=LR;');
    lines.push('  graph [overlap=false, splines=true, bgcolor="white"];');
    lines.push('  node [shape=box, style="rounded,filled", color="#475569", fontname="Arial"];');
    ...
    if (layers && Object.keys(layers).length > 0) {
        ...
        for (const layerName of layerNames) {
            ...
            for (const [groupName, groupNodes] of subgroups) {
                lines.push(`    subgraph cluster_${sanitizeId(layerName)}_${subgroupIndex} {`);
                lines.push(`      label=${quote(groupName)};`);
                lines.push('      style="rounded,dashed";');
                lines.push('      color="#cbd5e1";');
                for (const node of groupNodes.sort()) {
                    lines.push(`      ${quote(node)} [fillcolor=${quote(fillColor)}, label=${quote(node)}];`);
                }
                lines.push("    }");
                subgroupIndex += 1;
            }
        ...
        for (const node of ungrouped.sort()) {
            lines.push(`  ${quote(node)} [fillcolor="#f8fafc", label=${quote(node)}];`);
        }
    }
    else {
        for (const node of sortedNodes) {
    ...
    if (violations) {
        for (const violation of violations) {
            const key = `${simplifyPath(violation.from)}=>${simplifyPath(violation.to)}`;
    ...
    for (const from of sortedFromNodes) {
        const targets = [...(graph.adjacency.get(from) ?? new Set())].sort();
        for (const to of targets) {
            const fromKey = simplifyPath(from);
            ...
            if (violationSet.has(edgeKey)) {
                lines.push(`  ${quote(fromKey)} -> ${quote(toKey)} [color="red", penwidth=2.5];`);
            }
            else {
...
function buildLayerColorMap(layers) {
    const map = new Map();
    if (!layers)
        return map;
...
function buildSubgroups(nodes, layerName) {
...
function inferGroupName(node, layerName) {
...
function addToCluster(map, key, value) {
    if (!map.has(key)) {
        map.set(key, []);
    }
